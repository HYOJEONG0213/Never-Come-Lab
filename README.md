...
## 일화
### 무기 시스템 리팩토링 및 아키텍처 개선

**1. 아키텍처 개선 (MVP & Observer 패턴)**

- **문제 : 단일 책임 원칙 위배, 좀비 데이터 발생**
  - 버튼 UI가 무기 장착과 데이터 상태까지 직접 관리하여 **단일 책임 원칙**을 위반하고 결합도가 높았음.
  - ScriptableObject에 버튼의 상태를 저장하여 게임 재시작 시 정보가 남는 **좀비 데이터** 문제 발생
- **해결: MVP 패턴 도입해 역할을 분리함.**
  - **Model** : WeaponData에서 정적 데이터만 보유하도록 수정
  - **View** : WeaponUISlot에서 **Observer 패턴(Event)** 통해 상태 변화를 수동적으로 반영
  - **Present** : WeaponManager에서 무기의 생성, 교체, 상태 관리를 담당하는 시스템 구축

**2. 확장성 확보 (Template Method 패턴)**

- **문제**: Bullet 종류가 늘어날 때마다 **하드코딩**으로 인해 유지보수성과 확장성이 부족했음.
- **해결: 템플릿 메서드 패턴** 적용해 추상 클래스 BaseBullet에서 공통 로직을 처리함. 총알 종류별 효과는 파생 클래스에서 **고유 동작만 오버라이딩** 하도록 구조를 개선해 유연성을 확보함.

**3. 성능 최적화 (Data Structure 최적화)**

- **문제**: 무기 발사시 오브젝트 풀에서 프리팹을 찾기 위해 **배열을 순회**하는 방식(O(N))을 사용하여, 투사체 종류가 많아질수록 성능 저하됨.
- **해결**: 프리팹과 인덱스를 매핑하는 **Dictionary**를 도입해, 검색 알고리즘의 시간 복잡도를 O(1)로 단축해 빈번한 발사 로직의 오버헤드를 최소화함
...
씬 전환시 싱글톤 게임매니저 내 오브젝트가 사라지는 문제가 발생함. 싱글톤 남용이 결합도를 높여 문제가 됨을 깨닫고 필요할 때만 사용해야함을 알게됨. 최고 레벨 상태나 전역 접근이 필요한 경우에만 싱글톤 적용하고, 그 외에는 직접 참조하도록 수정함.
